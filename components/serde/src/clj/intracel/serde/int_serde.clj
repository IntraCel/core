  ...
  (serialize [this data]
    (let [buf ^ByteBuffer (ByteBuffer/allocate NUM_BYTES_IN_INTEGER)]
      (cond (> data Integer/MAX_VALUE)
            (throw (ex-info (format "[int-serde/serialize] Invalid argument. The data argument is larger than 32-bit integer allows. Value provided: %s, Max Allowed: %s" data Integer/MAX_VALUE)
