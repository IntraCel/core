...
(defrecord LongSerde [buf]
  ...
  (serialize [this data]
    (cond (> data Long/MAX_VALUE)
          ...
          (< data Long/MIN_VALUE)
          (throw (ex-info (format "[long-serde/serialize] Invalid argument. The data argument is beyond the minimum value of a 64-bit integer allows. Value provided: %s, Allowed: %s" data Long/MIN_VALUE)
                          {:cause #{:invalid-argument}}))
...
(defn create []
  (map->LongSerde {:buf ^ByteBuffer (ByteBuffer/allocate NUM_BYTES_IN_LONG)}))