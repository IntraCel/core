...
5. We'll create a datbase instance by calling teh ```kv-store/db``` function, using the reference to our ```KVStoreDbContextApi```. The second parameter is the name to give the database instance (```sg-1```). The third parameter gives the caller the ability to customize some of the behind-the-scenes implementation of the database instance. 
